  ...
  def main
    ...
    loop do
      ...
      if Input.trigger?(Input::DOWN)
        next if @encarray.length-1 < 7 && @navChoice < 7
        next if @encarray.length-1 > 6 && @encarray.length-1 <14 && @navChoice > 6 && @navChoice < 14
        @navChoice +=7
        navMon += 7
        @sprites["nav"].y += 64
        ...
        @sprites["navMon"].text = PBSpecies.getName(@encarray[navMon])
        @sprites["nav"].x -= 64
      elsif Input.trigger?(Input::RIGHT) && ((@navChoice !=6 && @navChoice !=13 && @navChoice !=20) || @navChoice != @encarray.length-1)
        @navChoice +=1
        navMon +=1
        @sprites["navMon"].text = PBSpecies.getName(@encarray[navMon])
        @sprites["nav"].x += 64
